...
def get_documents(documents: list, title: str, save: bool = False) -> dict:
    ...
    while next:
        try:
            ...
            utils.create_add_log(OUTPUT_PATH, "No more collections",
                                 title, 'documents.txt')
            next = False
            continue
        doc_next = True
        while doc_next:
            try:
                ...
                utils.create_add_log(OUTPUT_PATH,
                                     f"Downl. document for col {doc_title}",
                                     title, 'documents.txt')
                utils.create_add_log(OUTPUT_PATH, url,
                                     title, 'documents.txt')
            except IndexError:
                utils.create_add_log(OUTPUT_PATH,
                ...
                doc_next = False
                continue
            document = utils.get_response(url,
                                          headers={'Accept':
                                                   'application/json'})
            if document.status_code != 200:
                utils.create_add_log(OUTPUT_PATH,
                                     f"Error: {document.status_code}",
                                     title, 'documents.txt')
                continue
    ...
    if save:
        utils.save_dict_to_json(OUTPUT_PATH, es_ids, title, 'documents.json')
        utils.create_add_log(OUTPUT_PATH,
                             f"Saved file: documents.json to {title}",
                             title, 'documents.txt')
    utils.create_add_log(OUTPUT_PATH,
...
def get_documents_id(document: Response,
    ...
    while next:
        try:
            ...
            utils.create_add_log(OUTPUT_PATH,
                                 f"No more documents in col {title}",
                                 title, 'documents.txt')
            next = False
            continue
        document = utils.get_response(url,
                                      headers={'Accept':
                                               'application/json'})
        if document.status_code != 200:
            utils.create_add_log(OUTPUT_PATH, f"Error: {document.status_code}",
                                 title, 'documents.txt')
            continue
        document_json = document.json()
        document_es_id: str = document_json["es_id"]
...
def get_document_data(documents: dict,
    ...
    response = utils.post_response(url,
                                   headers={'Content-Type':
                                            'application/json'},
                                   data=utils.save_dict_to_json(
                                       data={"ids": keys}))
    if response.status_code != 200:
        utils.create_add_log(OUTPUT_PATH, f"Error: {response.status_code}",
                             title, 'data_response.txt')
        return None
    if save:
        utils.save_response(OUTPUT_PATH, response, title, 'data_response.json')
        utils.create_add_log(OUTPUT_PATH,
...
def tags_to_documents(documents: dict, title: str,
    ...
    while next:
        try:
            key = keys.pop()
            utils.create_add_log(OUTPUT_PATH,
                                 f"matching tags with document: {key}",
                                 title, 'documents_and_tags.txt')
        except IndexError:
            utils.create_add_log(OUTPUT_PATH,
                                 f"All documents matched w. tags {len(keys)}",
                                 title, 'documents_and_tags.txt')
            next = False
            continue
        ...
        while tag_next:
            try:
                tag = tag_list.pop()
                tag_id = tag.split('/')[-2]
                utils.create_add_log(OUTPUT_PATH, f"Matching id {tag_id}",
                                     title, 'documents_and_tags.txt')
            except IndexError:
                utils.create_add_log(OUTPUT_PATH,
                                     f"No more tags for document {key}",
                                     title, 'documents_and_tags.txt')
                tag_next = False
                continue
            try:
                ...
                utils.create_add_log(OUTPUT_PATH, f"{tag_label} added to list",
                                     title, 'documents_and_tags.txt')
        ...
        utils.create_add_log(OUTPUT_PATH,
                             f"Tags for document {key}: {tag_labels}",
                             title, 'documents_and_tags.txt')
        utils.sleeping(_SLEEP_TIME_DICT)
    if save:
        utils.save_dict_to_json(OUTPUT_PATH, documents, title,
