...
class DBOEData(BaseModel, DBOEUtils):
    """_summary_
    ...
    def download_collection(self, sorted_tags: dict) -> tuple[dict, dict]:
        ...
        WALK_WANT_API = self.load_env_var(_API_VAR_DATA)
    ...
    def download_and_sort_tags(self):
        ...
        tags = get_all_tags("tags__all")
        sorted_tags = sort_tags(tags, "tags__all", save=True)
        return sorted_tags
...
if __name__ == "__main__":
    ...
    for article in tqdm(articles, total=len(articles)):
        ...
        dd.col_id = col_id
        try:
            doc_date, doc_glob = dd.get_date_from_dir(INPUT_PATH, col_id,
                                                      "documents_and_tags")
            data_date, data_glob = dd.get_date_from_dir(INPUT_PATH, col_id,
                                                        "data_response")
            is_outdated = dd.is_file_outdated(doc_date, _EXPIRY_TIME)
        except IndexError:
            is_outdated = True
        dd.is_outdated = is_outdated
        if is_outdated:
            ...
            normdata_date, data_simplified_glob = dd.get_date_from_dir(
                INPUT_PATH,
                col_id,
                "simplified_normalized_data")
            print(f"Using existing simplified file: {data_simplified_glob}")
            corpus_date, data_corpus_glob = dd.get_date_from_dir(
                INPUT_PATH,
                col_id,
                "llm_corpus")
            print(f"Using existing data file: {data_corpus_glob}")
